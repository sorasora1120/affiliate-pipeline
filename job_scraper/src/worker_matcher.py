...
【対応体制】
国内外のデザイナー・エンジニアと連携したチーム体制で、企画から実装・納品までワンストップで対応しております。案件内容に応じて最適なメンバーをアサインするため、幅広いジャンル・技術要件に柔軟に対応可能です。
...
・進捗はこまめにご連絡し、音信不通にならないことをお約束します
・修正のご相談にも柔軟に対応いたします
...
PROPOSAL_TEMPLATE_QUOTE = """はじめまして。Web制作のご依頼を承っております、ソラと申します。
この度の募集内容「{title}」を拝見し、ぜひお力になれればと思いご連絡いたしました。
