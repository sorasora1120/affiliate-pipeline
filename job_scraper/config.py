...
WORKER_MATCH_EXCLUDE_KEYWORDS = [
    ...
    for k in os.getenv(
        ...
        # 2026-08-14: 56件を1件ずつ棚卸しして追加発見。プロジェクト単位の制作依頼
        # ではなく「人を雇う」形の募集（ディレクション業務・アシスタントスタッフ）、
        # 無償の持ち出しになりやすい依頼（コンペ形式、無料で判断してから発注）は
        # ワーカーへの発注が成立しない/割に合わないため除外する。
        "コンペ,アシスタントスタッフ,ディレクション業務,無料作成",
    ).split(",")
    if k.strip()
]
