...
KEYWORDS = [
    ...
    for k in os.getenv(
        ...
        "ホームページ作成,LP作成,ランディングページ作成,ネットショップ制作,"
        "WordPress制作,WordPress構築",
        # 2026-08-15: 「オンラインショップ制作/構築」を削除した。この2キーワードは
        # ココナラの緩いマッチングで事務作業・フリマアプリ出品代行・チャットレディ募集・
        # SNS運用代行まで拾ってしまい、除外キーワードで後追いしてもキリがないほど
        # 誤検知率が高かった（他のEC系キーワード＝Shopify/ECサイト構築等では
        # この規模の誤検知は出ていない）。ネットショップ/Shopify/ECサイト系の
        # キーワードで実質同じ需要はカバーできている。
    ).split(",")
    if k.strip()
]
...
WORKER_MATCH_EXCLUDE_KEYWORDS = [
    ...
    for k in os.getenv(
        ...
        "コンペ,アシスタントスタッフ,無料作成,"
        # 2026-08-15: 「ディレクション業務」は語尾が違う「〜LP作成ディレクション」
        # のような表記をすり抜けた（実際に誤検知としてビューアに出た）。「業務」を
        # 外し「ディレクション」単体にして表記ゆれをまとめて拾う。あわせて、
        # SNSの企画・アイデア出しのみを求める募集（Web制作ではない）も発見したため
        # 「アイデア出し」を追加。
        "ディレクション,アイデア出し,"
        # 2026-08-15: ページ数を2→5に増量した直後の棚卸しで発見。事務作業・
        # フリマアプリ出品代行・チャットレディ系の募集が「オンラインショップ」
        # キーワード経由で紛れ込んでいた（該当キーワード自体はJOB_KEYWORDSから
        # 削除済み）。Facebook広告の技術設定代行もWeb制作ではないため除外。
        "事務作業,出品代行,販売代行,CAPI設定",
    ).split(",")
    if k.strip()
]
